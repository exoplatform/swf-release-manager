...
function nexus_all_staging_repos {
  ...
  mvnCommand $1 nexus-staging:1.6.8:rc-list -DserverId=$STAGING_SERVER_ID -DnexusUrl=$STAGING_SERVER_URL
}
...
function nexus_close_staging_repo {
  ...
  if [ ${6:-false} = "true" ]; then
    mvnCommand $1 nexus-staging:1.6.8:rc-close nexus-staging:1.6.8:rc-release -DserverId=$maven_server_id -DnexusUrl=$nexus_url -DstagingRepositoryId=$2 -DstagingDescription=$5 -DstagingProgressTimeoutMinutes=10 2>&1 | tee -a ${LOGS_DIR}/infos.log
  else
    mvnCommand $1 nexus-staging:1.6.8:rc-close -DserverId=$maven_server_id -DnexusUrl=$nexus_url -DstagingRepositoryId=$2 -DstagingDescription=$5 -DstagingProgressTimeoutMinutes=10 2>&1 | tee -a ${LOGS_DIR}/infos.log
  fi
  if [ "$?" -ne "0" ]; then
    if [ ${6:-false} = "true" ]; then
...
function nexus_drop_staging_repo {
  ...
  mvn nexus-staging:1.6.8:rc-drop -DserverId=$maven_server_id -DnexusUrl=$nexus_url -DstagingRepositoryId=$1 -DstagingDescription=$4 2>&1 | tee -a ${LOGS_DIR}/infos.log
  if [ "$?" -ne "0" ]; then
...
function nexus_deploy_staged_repo {
  ...
  mvn nexus-staging:1.6.8:deploy-staged-repository -DnexusUrl=$nexus_url -DserverId=$maven_server_id -DrepositoryDirectory=${LOCAL_STAGING_DIR} -DstagingProfileId=$NEXUS_STAGING_PROFILE_ID -DstagingRepositoryId=$1 -Pexo-release,$maven_profile 2>&1 | tee -a ${LOGS_DIR}/infos.log
  if [ "$?" -ne "0" ]; then
...
function nexus_release_staging_repo {
  ...
  mvn nexus-staging:1.6.8:rc-release -DnexusUrl=$nexus_url -DserverId=$maven_server_id  -DstagingRepositoryId=$1 -DstagingDescription=$4 -DstagingProgressTimeoutMinutes=10 2>&1 | tee -a ${LOGS_DIR}/infos.log
  if [ "$?" -ne "0" ]; then
