...
source "${LIB_DIR}/utils/trycatch.sh"
source "${LIB_DIR}/process/release-catalog.sh"
source "${LIB_DIR}/process/release-status.sh"
source "${LIB_DIR}/process/git-clone.sh"
source "${LIB_DIR}/process/git-release.sh"
source "${LIB_DIR}/process/nexus-repos.sh"
source "${LIB_DIR}/process/maven-release.sh"
source "${LIB_DIR}/process/maven-dependencies.sh"
source "${LIB_DIR}/notifications/tasks.sh"
...
function exor_release_project {
  ...
  if [ "${versionSuffix:-}" != "" ] && [[ ! "${versionSuffix}" =~ ^[0-9]{8}([0-9][0-9])?$ ]]; then 
  ...
  if [ "${versionSuffix:-}" != "" ] && [[ "${versionSuffix}" =~ ^[0-9]{8}([0-9][0-9])?$ ]] && [[ "${issueId}" =~ ^[0-9]+$ ]]; then 
...
case $1 in
  ...
  "release-continue-from")
    if [[ "$2" =~ ^(nexus:(create|deploy|close|drop|release)|git:tagpush)$ ]]; then
