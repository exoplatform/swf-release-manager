...
. first, create the required resources (JSON catalog) for the Release and *synchronize* GitHub repositories
...
Based on the projects and the eXo Components diagram, create the JSON Catalog
for the release:
...
RUN cp -R ${EXO_CI_TMP_DIR}/conf/* ${CONFIG_DIR} \
        && cp -R ${EXO_CI_TMP_DIR}/lib/* ${SCRIPTS_DIR} \
        && cp -R ${EXO_CI_TMP_DIR}/data/* ${DATA_DIR} \
        && chmod u+x ${SCRIPTS_DIR}/*.sh \
        && chmod u+x ${SCRIPTS_DIR}/utils/*.sh \
        && cp ${EXO_CI_TMP_DIR}/bin/eXoR.sh ${SCRIPTS_DIR}/eXoR.sh \
        && chmod u+x ${SCRIPTS_DIR}/eXoR.sh
...
----
+
...
[source,shell]
----
$ docker build --build-arg ssh_passphrase='XXX' --build-arg nexus_token='XXXX' --build-arg gpg_passphrase='XXX' --build-arg jboss_password='XXX' -t exoplatform/release:RELMAN-1 .
----
+
. Use the `RELEASE-155` task to test a release with the `maven-sandbox-project`
+
[source,shell]
----
