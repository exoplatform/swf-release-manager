...
function maven_dependencies_update_before_release {
   ...
   local ARR
   mapfile -t ARR < <(jq -r '.[] | [.maven_property_version, .release.version, .release.current_snapshot_version, .release.next_snapshot_version] | join(",")' "${DATA_DIR}/catalog.json")

   if [ "${#ARR[@]}" -eq 0 ]; then
     printf '%s\n' "No maven properties to update."
   else
     for project in "${ARR[@]}"
       do
         ...
         local string_to_find="<${MAVEN_PROPERTY_VERSION}>${RELEASE_CURRENT_SNAPSHOT_VERSION}</${MAVEN_PROPERTY_VERSION}>"
         local string_to_write="<${MAVEN_PROPERTY_VERSION}>${RELEASE_VERSION}</${MAVEN_PROPERTY_VERSION}>"
   ...
   if [ "${#ARR[@]}" -eq 0 ]; then
     ...
     for project in "${ARR[@]}"
       do
         ...
         local RELEASE_CURRENT_SNAPSHOT_VERSION=${params[2]}
         local RELEASE_NEXT_SNAPSHOT_VERSION=${params[3]}
         local string_to_write="<${MAVEN_PROPERTY_VERSION}>${RELEASE_NEXT_SNAPSHOT_VERSION}</${MAVEN_PROPERTY_VERSION}>"
         local string_to_find="<${MAVEN_PROPERTY_VERSION}>${RELEASE_VERSION}</${MAVEN_PROPERTY_VERSION}>"
